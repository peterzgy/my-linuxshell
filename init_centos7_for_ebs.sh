#!/bin/bash
#########################################################################
# File Name: init_centos7_for_ebs.sh
# Author: zouguoyin
# mail: 995637339@qq.com
# Created Time: 2020年03月02日 星期三 18时08分48秒
#########################################################################
...
#阿里云使用安全组，可以不要iptables
yum install -y nmap unzip wget vim lsof xz net-tools  ntpdate ntp-doc psmisc
}
...
#创建普通用户
create_user(){
yum install    epel-release  vim   tree  lrzsz   telnet   wget  -y
groupadd  -g 300 ebs
useradd    -g    ebs    -u  600 ebs
#echo ebs:password|chpasswd
#切换普通用户创建目录
#mkdir   /home/ebs/{logs,soft,web,scripts,release,release-temp,package}
}
