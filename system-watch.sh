#########################################################################
# File Name: system-watch.sh
# Author: zouguoyin
# mail: 995637339@qq.com
# Created Time: 2020年12月29日 星期二 14时48分17秒
##########################################################################
#!/bin/bash

#usage: 需要使用root运行，脚本中 ifconfig、mpstat、iostat命令有依赖，需要安装相关依赖包
#安装依赖包
#yum -y install bc sysstat net-tools 

#ipaddress=`ip a|grep "global"|awk '{print $2}' |awk -F/ '{print $1}'`
ipaddress=`ip a|grep "global" | grep -v docker |grep -v "br-" | awk '{print $2}' | cut -d "/" -f 1`
...
get_crontab_content(){
   ...
   crontab -l|grep -Ev "^$"|awk -F ' ' '{ print $NF}' >>$crontab_content_log

   contrab_num=`crontab -l|grep -Ev "^$"|awk -F ' ' '{ print $NF}'|wc -l`
